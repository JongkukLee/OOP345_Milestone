  ...
  Factory(ItemManager& im, OrderManager& om, TaskManager& tm)
  {
    ...
    while (true) { // time loop
      ...
      for (auto& m : machineList) { // machine loop
        //cout << __LINE__ << " Job size: " << m.jobSize() << endl;   
        ...
        if (m.isSink()) {
          ...
          if (job.jobComplete()) // if job complete, it is finished, all done, game over, exists, the system
          {
        ...
        //cout << __LINE__ << " Job size: " << m.jobSize() << endl;
//        if (VERBOSE) cout << "Machine Name: " << m.name() << ", are we an installer?: " << m.getInstaller() 
//                                                          << " or remover?: " << m.getRemover() << "\n";
